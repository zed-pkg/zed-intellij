...
class ZedProjectService(
    ...
    fun refresh(showNotification: Boolean) {
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                ...
                val cliDiagnostic = if (cliState.available) {
                    emptyList()
                } else {
                    listOf(
                        ZedDiagnostic(
                            ...
                            summary = "Zed CLI is unavailable",
                            details = cliState.failure ?: "The 'zed' executable is not available on the IDE process PATH.",
                            packageRoot = root,
                            actions = listOf(
                ...
                val next = ZedWorkspaceSnapshot(
                    root = root,
                    scannedAt = Instant.now(),
                    packages = analysis.packages,
                    cli = cliState,
                    workspaceDiagnostics = analysis.workspaceDiagnostics + cliDiagnostic,
                )
                publish(next, showNotification)
            } catch (error: Exception) {
                logger.warn("Zed project scan failed", error)
                notify("Zed package scan failed", error.message ?: error.javaClass.simpleName, NotificationType.ERROR)
            } finally {
    ...
    fun runAction(action: ZedRecommendedAction, completion: (Boolean, String) -> Unit) {
        ...
        ApplicationManager.getApplication().executeOnPooledThread {
            try {
                ...
                ApplicationManager.getApplication().invokeLater {
                    completion(false, error.message ?: error.javaClass.simpleName)
                }
            }
        }
    }
    ...
    private fun publish(next: ZedWorkspaceSnapshot, showNotification: Boolean) {
        ApplicationManager.getApplication().invokeLater {
            ...
            if (showNotification) {
                ...
                notify(
                    "Zed package state refreshed",
                    "${next.packages.size} package(s), $errors error(s), $warnings warning(s).",
    ...
    private fun workspaceRoot(): Path = Path.of(project.basePath ?: System.getProperty("user.dir"))
        .toAbsolutePath()
        .normalize()

    private fun isZedStateFile(event: VFileEvent): Boolean {
        val name = event.file?.name ?: event.path.substringAfterLast('/')
        return name == ".zpkg.toml" || name == ".zpkg.lock" || name == "zed_modules"
    }
